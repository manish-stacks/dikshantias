...
const CourseCard: React.FC<{ course: CourseWithPurchase }> = ({ course }) => {
  ...
  const handleAction = () => {
    if (course.isPurchased) {
      ...
      router.push(`/video-courses/${course.slug}`);
    }
  };
  ...
  useEffect(() => {
    const fetchCoursesAndStatus = async () => {
      try {
        ...
        // Filter only recorded/video courses + active
        const recordedCourses: Course[] = (data.items || []).filter(
          (course: Course) => course.category === "recorded" && course.status === "active"
        );

        // Enrich with purchase status
        const coursesWithStatus = await Promise.all(
          recordedCourses.map(async (course) => {
        ...
        console.error("Error fetching video courses or purchase status:", error);
      } finally {
        setLoading(false);
      }
    };

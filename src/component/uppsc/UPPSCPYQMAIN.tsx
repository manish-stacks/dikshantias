...
const UPSCPYQ: React.FC = () => {
  ...
  const exam = pathname.includes("uppsc")
    ? "UPPSC"
    : pathname.includes("bpsc")
      ? "BPSC"
      : "UPSC";

  const type = pathname.includes("main") ? "MAINS" : "PRE";
    ...
    fetch("/api/admin/page-content")
      ...
      .then((res) => {
        const pyqList = res.filter(
          (item: PageContent) =>
            item.exam === exam && item.page === "PYQ" && item.status === true,
        );
  ...
  // ✅ FIND CONTENT
  const data = allData.find((item) => item.subject === selectedSubject);
          ...
          <div className="max-w-7xl mx-auto">
            <div className="bg-white rounded-2xl p-5 border shadow-xl">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Topic-wise PYQs with analysis and answer strategy.
              </p>

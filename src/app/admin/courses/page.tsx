...
export default function CoursePage() {
  ...
  const handleToggleActive = (id: string, currentStatus: boolean) => {
    setConfirmTitle(
      currentStatus ? "Deactivate this course?" : "Activate this course?",
    );
    setConfirmMessage(
    ...
    setConfirmAction(() => async () => {
      try {
        ...
        setCourses((prev) =>
          prev.map((course) =>
            course._id === id
              ? { ...course, active: data.course.active }
              : course,
          ),
        );
  ...
  const filteredCourses = courses.filter((course) => {
    ...
    const matchesLanguage = filterLanguage
      ? course.languages.includes(filterLanguage)
      : true;
    const matchesPrice =
          ...
          <div className="relative">
            ...
            >
              ...
              {Array.from(
                new Set(
                  courses.flatMap((c) =>
                    c.languages.split(",").map((lang) => lang.trim()),
                  ),
                ),
              ).map((lang, index) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse bg-white rounded-2xl shadow-lg overflow-hidden">
            <thead className="bg-gray-100 text-gray-700 text-sm uppercase tracking-wide font-semibold">
              <tr>
                <th className="py-4 px-5 text-left border-b border-gray-200 rounded-tl-2xl">
                  Image
                </th>
                  ...
                  >
                    ...
                    <td className="py-3 px-5 text-center">
                      <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        <div
                          onClick={() =>
                            handleToggleActive(course._id, course.active)
                          }
                        ...
                        >
                          <span
                            className={`bg-white w-5 h-5 rounded-full shadow-md transform transition-transform duration-300 ${
                              course.active ? "translate-x-6" : "translate-x-0"
                            }`}
                          ></span>
                        </div>
                      </label>
